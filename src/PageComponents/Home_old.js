...
function getPageList(totalPages, currentPage, paginationSize) {
  if (paginationSize < 5) {
    throw new Error("paginationSize must be at least 5");
  }
  ...
  const sideWidth = paginationSize < 9 ? 1 : 2;
  const leftWidth = (paginationSize - sideWidth * 2 - 3) >> 1;
  const rightWidth = (paginationSize - sideWidth * 2 - 2) >> 1;

  if (totalPages <= paginationSize) {
    return range(1, totalPages);
  }

  if (currentPage <= paginationSize - sideWidth - 1 - rightWidth) {
    return range(1, paginationSize - sideWidth - 1).concat(
      0,
...
const [currentPage, setCurrentPage] = useState(1);
  const numberOfItems = document.querySelectorAll(".tab-pane.stt-tab-list-ofservice.active .modern-search-result.service-list-wrapper .row .col-12.col-sm-12.col-md-6.col-lg-6.col-xl-4").length;
  ...
  useEffect(() => {
    // Logic to handle showing items for the current page
    const startIndex = (currentPage - 1) * limitPerPage;
    const endIndex = startIndex + limitPerPage;
    const allItems = document.querySelectorAll(".tab-pane.stt-tab-list-ofservice.active .modern-search-result.service-list-wrapper .row .col-12.col-sm-12.col-md-6.col-lg-6.col-xl-4");
    
    allItems.forEach((item, index) => {
      if (index >= startIndex && index < endIndex) {
        item.style.display = "block"; // Show items within the current page range
      } else {
        item.style.display = "none"; // Hide items outside the current page range
      }
    });
												...
												<div className="item service-border st-border-radius">
													...
													<div className="content-item">
														<div className="content-inner has-matchHeight" style={{ height: '75.2px' }}>
														  ...
														  <div className="sub-title d-flex align-items-center" itemProp="address" itemScope itemType="https://schema.org/PostalAddress">
														    <span itemProp="streetAddress">New York City helloo</span>
														  </div>
														</div>
														<div className="section-footer">
															<div className="reviews" itemProp="starRating" itemScope="" itemType="https://schema.org/Rating">
												...
												<div className="item service-border st-border-radius">
													...
													<div className="content-item">
														<div className="content-inner has-matchHeight" style={{ height: '75.2px' }}>
														  ...
														  <h3 className="title" itemProp="name">
														    <Link to="https://modmixmap.travelerwp.com/st_hotel/castello-casole-hotel/" className="c-main">Castello Casole Hotel</Link>
														  </h3>
														  <div className="sub-title d-flex align-items-center" itemProp="address" itemScope itemType="https://schema.org/PostalAddress">
														    <span itemProp="streetAddress">New York City</span>
														  </div>
								...
								<div className="panigation-list-new-style pagination moderm-pagination justify-content-center" data-stt_service="st_hotel" data-action_service="st_filter_hotel_ajax" data-order="ASC" data-orderby="post__in" data-st_item_row="3" data-st_item_row_tablet="2" data-st_item_row_tablet_extra="2" data-posts_per_page="8">
									<div className="pagination">
								        ...
								        {pageList.map((item, index) => (
								          ...
								          <button
								            key={index}
								            className={`btn page-item ${item ? "current-page" : "disabled"} ${item === currentPage ? "btn-primary active" : ""}`}
								            onClick={(event) => {
											      event.preventDefault();
											      event.stopPropagation();
											      setCurrentPage(item);
